...
-- 2022-05-29,DSabat,Created File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_DSabat')
	 Begin 
	  Alter Database [Assignment07DB_DSabat] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_DSabat;
	 End
	Create Database Assignment07DB_DSabat;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_DSabat;
...
ORDER BY 
	ProductName, vInventories.InventoryDate;
go
...
ORDER BY 
	CategoryName, vInventories.InventoryDate;
go
...
AS
    SELECT TOP 100000
		...
		[InventoryCount],
        [PreviousMonthCount] = 
        IIF(Month(InventoryDate) = 1, 0, Lag(InventoryCount) Over (Order By ProductName))
    FROM
        vProductInventories;
go
